...
def avg_diags(m):
    ...
    principal_diagonal = []
    for index in range(1, len_matrix, 2):
        if m[index][index] >= 0:
            principal_diagonal.append(m[index][index])
            
    if len(principal_diagonal) == 0:
        avg1 = -1
    else:
        avg1 = round(sum(principal_diagonal) / len(principal_diagonal))
    
    secondary_diagonal = []
    row = len_matrix -1
    for col in range(0, len_matrix,2):
        if m[row][col] < 0:
            secondary_diagonal.append(m[row][col])
        row -= 2 
    
    if len(secondary_diagonal) == 0:
        avg2 = -1 
    else:
        avg2 = round( (sum(secondary_diagonal) / len(secondary_diagonal)) * (-1) )
